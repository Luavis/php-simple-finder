						...
						if(!in_array($file_ext, $exclude_ext))
						{
							...
							foreach($pathComponent as $key => $value) {
								 if($value == '.' || $value == '')
									...
									$link .= ('/' . rawurlencode(iconv('euc-kr', 'utf-8', $value)));
								 }
							}
							
							$link .= ('/' . rawurlencode(iconv('euc-kr', 'utf-8', $val)));
...
HTML;
	...
	if(!empty($url_folder))
	{
		...
		for($i=0;$i<$folders_in_url_count;$i++)
		{
			...
			$temp = substr($temp, 1);
			echo '<a href="'.$this_file_name.'?folder='.urlencode(base64_encode($temp)).'">'.iconv('euc-kr','utf-8', $folders_in_url[$i]).'</a>/';
		}
	}
...
HTML;
	if(!empty($url_folder))
	{
		...
		<table width="100%" cellpadding="5" cellspacing="5">
			<tr>
				<td bgcolor="#DDDDDD" valign="middle"><a href="'.$this_file_name.'?folder='.urlencode(base64_encode($temp)).'">ㄴ Upper directory</a></td>
			</tr>
		</table>';
	...
	foreach($folders_sorted as $key => $val)
	{
		echo 
			'<tr class="folder_bg">
				<td width="60%">
					<img src="folder.png">
					<a href="'.$this_file_name.'?folder='.urlencode(base64_encode($folders['link'][$key])).'">'.iconv('euc-kr', 'utf-8', $folders['name'][$key]).'</a></div>
				</td>
				<td width="20%">';
	...
	foreach($files_sorted as $key => $val)
	{
		...
		echo ' <a href="'.$file_link.'">'.iconv('euc-kr','utf-8', $files['name'][$key]).'</a></div>';
